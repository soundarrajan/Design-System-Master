import {AfterViewInit, Component, ViewChild, ViewContainerRef, ElementRef, Renderer2} from "@angular/core";
...
@Component({
    ...
    <div class="editbubble dateTime-picker">
        <mat-form-field class="no-shadow-form">
            ...
            <mat-datepicker-toggle matSuffix [for]="picker" id="trigger" #trigger style="display:none;"></mat-datepicker-toggle>
            <mat-datepicker id="noshadow" #picker class="custom-calendar"></mat-datepicker>
        </mat-form-field>
        <input placeholder=""
               [(ngModel)]="dateTime"
               [owlDateTimeTrigger]="timer" [owlDateTime]="timer" class="timer-trigger" >
        <owl-date-time #timer [pickerType]="'timer'" panelClass="timer-picker" class="timer-pick" (afterPickerClosed)="closed();"></owl-date-time>
        <div class="btn-block" style="position:absolute; bottom: 12px;left: 0;border-top: 1px solid rgba(0,0,0,.12);padding-top: 7px;">
            <button mat-button style="position: relative;top: 3px;float: left;left: 22px;">CANCEL</button>
            <button class="blue-button" mat-button style="width:100px;position: relative;top: 3px;float: right;right: 22px;">PROCEED</button> 
        </div>
    </div>
    </div>
...
export class AGGridDateTimePickerComponent implements ICellEditorAngularComp, AfterViewInit {
    private params: any;
    ...
    ngAfterViewInit() {
        ...
        document.getElementsByClassName("mat-datepicker-content")[0].classList.add("parent-datepicker");
