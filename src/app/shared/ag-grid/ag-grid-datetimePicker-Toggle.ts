import { AfterViewInit, Component, ViewChild, ViewContainerRef, ElementRef, Output,EventEmitter, Input } from '@angular/core';
...
@Component({
  ...
  template: `
  <div (click)="$event.stopPropagation();picker.open();" style="cursor:pointer;float: left;position:relative;font-size: 12px;color: #ffffff;
  font-weight: 500;">
  <div class="quoteByContainer">
  <mat-form-field class="quoteByMatField">
    <input style="cursor:pointer;width:65px;float:left;height: 17px !important;text-align:left;color:#FFFFFF" matInput class="date-trigger" [formControl]="initialDate"
    [ngModel]="initialDate.value" [matDatepicker]= "picker" (dateChange)="dateChanged($event)"
    #datetrigger>
    </mat-form-field>
    <div  style="height:24px;float:right;line-height:15px;width:30px;position: absolute;
    right: -1px;">{{timeValue}}</div>
    </div></div>
    <mat-datepicker-toggle matSuffix [for]="picker">
    <mat-icon matDatepickerToggleIcon class="mini-dateIcon"></mat-icon>
    </mat-datepicker-toggle>
    <mat-datepicker [panelClass]="dark?'new-datepicker datepicker-darktheme':'new-datepicker'" #picker (opened)="opened()"></mat-datepicker>

    <input #timepicker
               [(ngModel)]="timerValue" (dateTimeChange)="onChange($event)"
        ...
        <owl-date-time  [pickerType]="'timer'" #dt [panelClass]="dark?['timerPanelClass', 'darktheme']:'timerPanelClass'" (afterPickerClosed)="timepickerClosed()" (afterPickerOpen)="timepickerOpened()"></owl-date-time>
        </div>
  `,
  providers: [
...
export class AgGridDatetimePickerToggleComponent implements ICellEditorAngularComp, AfterViewInit {
  private params: any;
 ...
 // matDate.setValue('1/1/2021');
  ...
  dateChanged(event) {
   ...
   this.picker.close = () => { };
   this.picker['_popupComponentRef'].instance._calendar.monthView._createWeekCells()
   setTimeout(() => {
