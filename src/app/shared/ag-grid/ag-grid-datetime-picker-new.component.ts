...
@Component({
  ...
  <div style="width: 63%;margin:0 auto">
    <input style="width:67%;height: 17px !important;top:1px;text-align:left;" matInput class="date-trigger" [formControl]="initialDate"
    [matDatepicker]= "picker" (dateChange)="dateChanged($event)"
    #datetrigger>
    <div  style="height:25px;float:right;line-height:26px;width33%">{{timeValue}}</div>
    </div></div>
    ...
    <input placeholder="Date Time:" #timepicker
               [(ngModel)]="timerValue" (dateTimeChange)="onChange($event)"
               [owlDateTimeTrigger]="dt" [owlDateTime]="dt" style="border: none">
        <div class="time-pick-container">
        <owl-date-time  [pickerType]="'timer'" #dt panelClass="timerPanelClass" (afterPickerClosed)="timepickerClosed()" (afterPickerOpen)="timepickerOpened()"></owl-date-time>
        </div>
  `
})
export class AgGridDatetimePickerNewComponent implements ICellEditorAngularComp, AfterViewInit {
  private params: any;
  valueField: any;
  oldCellValue: string;
  timeValue: any;
