...
@Component({
    ...
    `<mat-form-field style="width:185px">
        <input matInput [placeholder]="this.placeHolder"  name="{{this.inputElId}}" id="{{this.inputElId}}" (input)="checkAutoComplete($event)"
            [matAutocomplete]="auto" [(ngModel)]="valueField" [value]="valueField" #input>
    </mat-form-field>

    <mat-autocomplete #auto="matAutocomplete" [displayWith]="displayFn.bind(this)" class="dark-autocomplete" style="margin-top: 5px;">
        <mat-option *ngFor="let item of this.selectList " matTooltip="{{item}}" (onSelectionChange)="updateValueFromSelectionChange($event)" [value]="item[controlName]">
            {{ item}}</mat-option>
    </mat-autocomplete>`,
})
export class AgGridLookupEditor implements ICellEditorAngularComp, AfterViewInit {
    ...
    getValue(): any {
        if(this.isValueSelectedFromDropdown == true)
