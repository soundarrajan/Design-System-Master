...
@Component({
    ...
    template: `<div ref="tabBody" class="ag-tab-body">
    <div class="ag-filter">
       <div>
          <div class="ag-filter-body-wrapper">
             <div ref="ag-filter-loading" class="loading-filter ag-hidden">Loading...</div>
             <div>                
                <div class="ag-filter-header-container">                   
                    <select class="ag-filter-select" id="filterType" [(ngModel)]="condition">
                ...
                <div>
                    <div class="ag-input-text-wrapper search-container" id="ag-mini-filter">
                        <input type="text" placeholder="Search..." #input [ngModel]="searchText" (ngModelChange)="onChange($event)">
                        <select placeholder="Select" [(ngModel)]="selectedUnit" name="unit" (ngModelChange)="onChangeUnit($event)">
                            <option  *ngFor="let unit of units" [value]="unit.value">{{unit.name}}</option>
                        </select>
                     </div>                     
                </div>
             </div>
 ...
 <div class="ag-filter-apply-panel" id="applyPanel">
    <button type="button" id="clearButton" class="fbtn" (click)="onClear()">Clear Filter</button>
    <button type="button" id="applyButton" class="fbtn" (click)="onApply()">Apply Filter</button>
</div>
    `, styles: [
        `
...
export class AggridCustomFilter implements IFilterAngularComp {
    ...
    isFilterActive(): boolean {
        return (this.searchText !== null && this.searchText !== undefined && this.searchText !== '') || (this.selectedUnit !== null && this.selectedUnit !== undefined && this.selectedUnit !== '')
    }
    ...
    doesFilterPass(params: IDoesFilterPassParams): boolean {
        ...
        if(this.selectedUnit!=null && this.selectedUnit!='' && this.selectedUnit!= undefined){
            if(params.data[this.params.secondaryColId]==this.selectedUnit){                
                if(this.searchText !== null && this.searchText !== undefined && this.searchText !== ''){
                    return this.validateWithCondition(Number(params.data[this.params.primaryColId]));
                }
                else{
                    return true;
                }               
