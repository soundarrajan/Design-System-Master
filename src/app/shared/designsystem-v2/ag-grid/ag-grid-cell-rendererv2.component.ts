...
import { PreviewRfqPopupComponent } from "src/app/shiptech-spot-negotiation/spot-negotiation-popups/preview-rfq-popup/preview-rfq-popup.component";
import { SupplierCommentsNewPopupComponent } from "src/app/shiptech-spot-negotiation/spot-negotiation-popups/supplier-comments-new-popup/supplier-comments-new-popup.component";
...
@Component({
    ...
    justify-content: center;" (click)="sellerratingpopup(params.data.genRating,params.data.genPrice,params.data.portRating,params.data.portPrice)">
...
export class AGGridCellRendererV2Component implements ICellRendererAngularComp {
    ...
    additionalcostpopup(data){
      ...
      sellerratingpopup(genRating,genPrice,portRating,portPrice){
        //alert(genRating);
        const dialogRef = this.dialog.open(SellerratingpopupComponent, {
          width: '1164px',
          height: '562px',
          panelClass: 'additional-cost-popup',
          data:{gRating:genRating,gPrice:genPrice,pRating:portRating,pPrice:portPrice}
        });  
        dialogRef.afterClosed().subscribe(result => {
        });
      }
