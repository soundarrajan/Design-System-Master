...
export class JsonSchemaFormService {
  private controlChangeBus: Subject<ControlChangeEvent> = new Subject<ControlChangeEvent>();
  ...
  send(event: any, data?: any) {
    this.controlChangeBus.next({ event, data });
  }

  receive(key: any): Observable<any> {
    return this.controlChangeBus.asObservable().filter(res => res.event === key);
  }
  constructor() {
  ...
  getFormGroupControlValue(keys){ 
    return this.formGroup.controls[keys];
  }
  getAllFormGroupControl(){
    return this.formGroup.controls
  }
  setModelValuesFromResponse(selectedValues){    
    var keys = Object.keys(selectedValues);  
    for (let k = 0; k < keys.length; k++) {    
      if(this.formGroup.controls[keys[k]] !== undefined){
        this.formGroup.controls[keys[k]].setValue(selectedValues[keys[k]]!== null ? selectedValues[keys[k]]: '  ');      
      }
    }
  }
  ...
  checkRequiredFields(type = ''): boolean {
    var returnVal = true;
    var keys = Object.keys(this.formGroup.controls);
    for (let i = 0; i < keys.length; i++) {
      if (this.formGroup.controls[keys[i]].status.toLowerCase() === 'invalid') {
        if (
          type === 'numberinput' &&
          (this.formGroup.controls[keys[i]].value !== undefined && this.formGroup.controls[keys[i]].value !== null && this.formGroup.controls[keys[i]].value === -1)
        )
          returnVal = true;
  ...
  validateData(newValue: any, updateSubscriptions = true): void {
    ...
    if (updateSubscriptions) {
      ...
      this.isValidChanges.next(this.isValid && this.checkRequiredFields());
  ...
  buildSchemaFromData(requiredArr, data?: any, requireAllFields = false): any {
    if (data) {
      return buildSchemaFromData(data, requireAllFields);
    }
    var schema = buildSchemaFromData(this.formValues, requireAllFields);
  ...
  parseExpression(expression = '', value: any = {}, values: any = {}, key: number | string = null, tpldata: any = null) {
    if (typeof expression !== 'string') {
      return '';
    }
    const index = typeof key === 'number' ? key + 1 + '' : key || '';
    expression = expression.trim();
    if ((expression[0] === "'" || expression[0] === '"') && expression[0] === expression[expression.length - 1] && expression.slice(1, expression.length - 1).indexOf(expression[0]) === -1) {
  ...
  setArrayItemTitle(parentCtx: any = {}, childNode: any = null, index: number = null): string {
    ...
    const text = JsonPointer.getFirst(
      isArrayItem && childNode.type !== '$ref'
        ? [[childNode, '/options/legend'], [childNode, '/options/title'], [parentNode, '/options/title'], [parentNode, '/options/legend']]
        : [[childNode, '/options/title'], [childNode, '/options/legend'], [parentNode, '/options/title'], [parentNode, '/options/legend']]
    );
    if (!text) {
      return text;
    }
  ...
  initializeControl(ctx: any, bind = true): boolean {
    ...
    if (isEmpty(ctx.options)) {
      ctx.options = !isEmpty((ctx.layoutNode || {}).options) ? ctx.layoutNode.options : _.cloneDeep(this.formOptions);
    }
    ...
    if (ctx.formControl) {
      ...
      ctx.options.errorMessage = ctx.formControl.status === 'VALID' ? null : this.formatErrors(ctx.options, ctx.formControl.errors, ctx.options.validationMessages);
      ctx.options.showErrors = this.formOptions.validateOnRender === true || (this.formOptions.validateOnRender === 'auto' && hasValue(ctx.controlValue));
      ctx.formControl.statusChanges.subscribe(status => {
        //
        ctx.options.errorMessage =
          status === 'VALID'
            ? null
            : ctx['controlType'] === 'numberinput' && ctx.controlValue === -1
            ? null
            : this.formatErrors(ctx.options, ctx.formControl.errors, ctx.options.validationMessages);
        ctx.status = ctx['controlType'] === 'numberinput' && ctx.controlValue === -1 ? 'VALID' : ctx.status;        
        this.isValidChanges.next(this.checkRequiredFields(ctx['controlType']));
      });
      ctx.formControl.valueChanges.subscribe(value => {
        // if (ctx["controlType"] === "dropdown" || ctx["controlType"] === "lookup") {
        //   if (ctx.selectList.length > 0 && typeof ctx.selectList[0][ctx.options.controlValue] === "number" && typeof value !== "number") {
        //     ctx.controlValue = parseInt(value);
        //   }
        //   else if (ctx.selectList.length > 0 && typeof ctx.selectList[0][ctx.options.controlValue] === "number" && value === -1) {
        //     ctx.controlValue = -1;
        //     ctx.status = "INVALID"
        //   }
        // }
        // else       
        if (ctx['controlType'] === 'numberinput' && value == -1) {
          ctx.controlValue = -1;
          ctx.status = 'VALID';
        } else if (!_.isEqual(ctx.controlValue, value)) {
  ...
  formatErrors(options: any, errors: any, validationMessages: any = {}): string {
    if (isEmpty(errors)) {
      return null;
    }
    ...
    const addSpaces = string => string[0].toUpperCase() + (string.slice(1) || '').replace(/([a-z])([A-Z])/g, '$1 $2').replace(/_/g, ' ');
    const formatError = error =>
      typeof error === 'object'
        ? Object.keys(error)
            .map(key => (error[key] === true ? addSpaces(key) : error[key] === false ? 'Not ' + addSpaces(key) : addSpaces(key) + ': ' + formatError(error[key])))
            .join(', ')
            ...
            ? // Does error message have any {{property}} placeholders?
              ...
              : // Replace {{property}} placeholders with values
                Object.keys(errors[errorKey]).reduce(
                  (errorMessage, errorProperty) => errorMessage.replace(new RegExp('{{' + errorProperty + '}}', 'g'), errors[errorKey][errorProperty]),
                  validationMessages[errorKey]
                )
            : errorKey === 'required'
            ? options.title + ' is a required field'
            : errorKey === 'min'
            ? options.title + ' cannot be lesser than ' + errors[errorKey].min.toString()
            : errorKey === 'max'
            ? options.title + ' cannot be greater than ' + errors[errorKey].max.toString()
  ...
  hideParentNode(ctx: any) {
    var divs = document.getElementsByClassName('form-flex-item ng-star-inserted');
    for (let i = 0; i < divs.length; i++) {
      var currentDiv: any = divs[i];
      var firstChild: any = currentDiv.firstChild;
      if (firstChild.hasAttribute('ng-reflect-layout-index') && firstChild.getAttribute('ng-reflect-layout-index') === ctx.layoutIndex.toString()) {
        currentDiv.style.display = 'none';
      }
    }
  }

  showParentNode(ctx: any) {
    var divs = document.getElementsByClassName('form-flex-item ng-star-inserted');
    for (let i = 0; i < divs.length; i++) {
      var currentDiv: any = divs[i];
      var firstChild: any = currentDiv.firstChild;
  ...
  removeItem(ctx: any): boolean {
    if (!ctx.layoutNode || !isDefined(ctx.layoutNode.dataPointer) || !hasValue(ctx.dataIndex) || !hasValue(ctx.layoutIndex)) {
      return false;
    }
