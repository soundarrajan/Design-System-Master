import { ChangeDetectionStrategy, ChangeDetectorRef, Component, EventEmitter, forwardRef, Input, Output, OnChanges, OnInit } from '@angular/core';
...
import { hasValue, inArray, isArray, isEmpty, isNumber, isObject } from './shared/validator.functions';
...
@Component({
  ...
  template: `
    ...
    <form class="json-schema-form" id="myForm" style="display:none;" (ngSubmit)="submitForm()">
...
export class JsonSchemaFormComponent implements ControlValueAccessor, OnChanges, OnInit {
  ...
  // Recommended inputs
  required = []; // The JSON Schema
  @Input('required') set _required(val) {
    if (val !== undefined && val !== null && val.length > 0) this.required = val;
  }
  ...
  public ngModel: any; // Alternate input for Angular forms
  @Input('ngModel') set _ngModel(val) {
    this.ngModel = val;
  } // Alternate input for Angular forms
  public resetForm = false;
  @Input('resetForm') set _resetForm(val) {
    if (val) {
      this.initializeData();
      this.activateForm();
    }
  ...
  @Output() jsonFormcontrolValues = new EventEmitter<any>(); // cahnge events values
  ...
  jsonFormControlBindValues(event){
     this.jsonFormcontrolValues.emit(event);
  }
   ...
   */
  initializeForm() {
    if (this.schema || this.layout || this.data || this.form || this.model || this.JSONSchema || this.UISchema || this.formData || this.ngModel || this.jsf.data) {
  ...
  private initializeSchema() {
    ...
    if (!isEmpty(this.jsf.schema)) {
      ...
      if (hasOwn(this.jsf.schema, 'type') && this.jsf.schema.type !== 'object') {
        ...
        if (isObject(this.jsf.schema.properties) || isObject(this.jsf.schema.patternProperties) || isObject(this.jsf.schema.additionalProperties)) {
          this.jsf.schema.type = 'object';
  ...
  private activateForm() {
    ...
    if (isEmpty(this.jsf.schema)) {
      ...
      if (!isEmpty(this.jsf.formValues)) {
        this.jsf.buildSchemaFromData(this.required);
      }
    }
    ...
    if (this.jsf.formGroup) {
      ...
      if (!isEmpty(this.jsf.formValues) && this.jsf.formOptions.setSchemaDefaults !== true && this.jsf.formOptions.setLayoutDefaults !== true) {
      ...
      this.jsf.dataChanges.subscribe(data => { this.jsonFormControlBindValues(this.jsf.getAllFormGroupControl());
