...
export class JsonValidators {
   ...
   * @returnIValidatorFn} - returns the 'required' validator function itself
   */
  ...
  static type(requiredType: SchemaPrimitiveType | SchemaPrimitiveType[]): IValidatorFn {
    ...
    return (control: AbstractControl, invert = false): ValidationErrors | null => {
      ...
      const isValid = isArray(requiredType) ? (<SchemaPrimitiveType[]>requiredType).some(type => isType(currentValue, type)) : isType(currentValue, <SchemaPrimitiveType>requiredType);
  ...
  static format(requiredFormat: JsonSchemaFormatNames): IValidatorFn {
    ...
    return (control: AbstractControl, invert = false): ValidationErrors | null => {
      ...
      if (isString(currentValue)) {
        ...
        isValid = ['date', 'time', 'date-time'].includes(requiredFormat) && Object.prototype.toString.call(currentValue) === '[object Date]';
      }
  ...
  static multipleOf(multipleOfValue: number): IValidatorFn {
    ...
    return (control: AbstractControl, invert = false): ValidationErrors | null => {
      ...
      let currentValue = control.value;
      let isValid = isNumber(currentValue) && currentValue % multipleOfValue === 0;
   ...
   * @returnIValidatorFn} - single combined validator function
   */
   ...
   * @returnIValidatorFn} - new validator function that returns opposite result
   */
  ...
  static compose(validators: IValidatorFn[]): IValidatorFn {
    ...
    return (control: AbstractControl, invert = false): ValidationErrors | null => _mergeErrors(_executeValidators(control, presentValidators, invert));
  }
   ...
   * @returnAsyncIValidatorFn} - single combined async validator function
   */
