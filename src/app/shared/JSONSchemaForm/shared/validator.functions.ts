 ...
 * @param  any } value - the value to check
 * @param  any = false } strict - if truthy, also checks JavaScript tyoe
 * @returnboolean } - true if number, false if not
 */
...
export function isBoolean(value, option: any = null) {
  ...
  return value === true || value === 1 || value === 'true' || value === '1' || value === false || value === 0 || value === 'false' || value === '0';
}
...
export function isDate(item: any): boolean {
  return typeof item === 'object' && Object.prototype.toString.call(item) === '[object Date]';
}
...
export function toSchemaType(value, types) {
  ...
  if (((<SchemaPrimitiveType[]>types).includes('number') || (<SchemaPrimitiveType[]>types).includes('integer')) && !(<SchemaPrimitiveType[]>types).includes('null')) {
...
export function inArray(item, array, allIn = false) {
  ...
  return isArray(item) ? item[allIn ? 'every' : 'some'](subItem => array.includes(subItem)) : array.includes(item);
}
