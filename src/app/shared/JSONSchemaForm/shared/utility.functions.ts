import { hasValue, inArray, isArray, isDefined, isObject, isEmpty, isMap, isSet, isString, PlainObject } from './validator.functions';
 ...
 * @returnstring | string[] | Set<string>} - Combined classes
 */
export function addClasses(oldClasses: string | string[] | Set<string>, newClasses: string | string[] | Set<string>): string | string[] | Set<string> {
 ...
 * @returnObject|Array|string|number|boolean|null} - The copied object
 */
 ...
 * @returnvoid}
 */
export function forEach(object: any, fn: (v: any, k?: string | number, c?: any, rc?: any) => any, recurse: boolean | string = false, rootObject: any = object, errors = false): void {
  if (isEmpty(object)) {
    return;
  }
 ...
 * @param function} fn - The iterator funciton to call on each item
 * @param boolean = false} errors - Show errors?
 * @returnObject | Array} - The resulting object or array
 */
export function forEachCopy(object: any, fn: (v: any, k?: string | number, o?: any, p?: string) => any, errors = false): any {
  if (!hasValue(object)) {
    return;
  }
 ...
 * @returnboolean} - true if object has property, false if not
 */
export function hasOwn(object: any, property: string): boolean {
  if (!object || !['number', 'string', 'symbol'].includes(typeof property) || (!isObject(object) && !isArray(object) && !isMap(object) && !isSet(object))) {
    return false;
  }
 ...
 * @returnPlainObject} - Returns targetObject
 */
...
export function toTitleCase(input: string, forceWords?: string | string[]): string {
  ...
  let forceArray: string[] = ['a', 'an', 'and', 'as', 'at', 'but', 'by', 'en', 'for', 'if', 'in', 'nor', 'of', 'on', 'or', 'per', 'the', 'to', 'v', 'v.', 'vs', 'vs.', 'via'];
  ...
  return input.replace(/[A-Za-z0-9\u00C0-\u00FF]+[^\s-]*/g, (word, idx) => {
    if (!noInitialCase && word.slice(1).search(/[A-Z]|\../) !== -1) {
      ...
      if (!forceWord) {
        ...
        (idx === 0 || idx + word.length === input.length || prevLastChar === ':' || input[idx - 1].search(/[^\s-]/) !== -1 || (input[idx - 1] !== '-' && input[idx + word.length] === '-'))
      ) {
