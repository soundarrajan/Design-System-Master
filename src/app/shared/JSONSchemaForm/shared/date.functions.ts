...
export function dateToString(date, options: any = {}) {
  ...
  return dateFormat
    ...
    .replace(/YY/gi, (date.getFullYear() + '').slice(-2))
    .replace(/MMMM/gi, longMonths[date.getMonth()])
    .replace(/MMM/gi, shortMonths[date.getMonth()])
    .replace(/MM/gi, ('0' + (date.getMonth() + 1)).slice(-2))
    .replace(/M/gi, date.getMonth() + 1 + '')
    .replace(/DDDD/gi, longDays[date.getDay()])
    .replace(/DDD/gi, shortDays[date.getDay()])
    .replace(/DD/gi, ('0' + date.getDate()).slice(-2))
    .replace(/D/gi, date.getDate() + '')
    .replace(/S/gi, ordinal(date.getDate()));
}
