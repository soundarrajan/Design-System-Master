...
import { hasValue, inArray, isArray, isEmpty, isDate, isObject, isDefined, isPrimitive, toJavaScriptType, toSchemaType, SchemaPrimitiveType } from './validator.functions';
import { forEach, hasOwn } from './utility.functions';
import { Pointer, JsonPointer } from './jsonpointer.functions';
import { JsonValidators } from './json.validators';
import { combineAllOf, getControlValidators, getSubSchema, removeRecursiveReferences } from './json-schema.functions';
 ...
 * @returnany} -
 */
export function buildFormGroupTemplate(jsf: any, nodeValue: any = null, setValues = true, schemaPointer = '', dataPointer = '', templatePointer = '') {
  const schema = JsonPointer.get(jsf.schema, schemaPointer);
  if (setValues) {
    if (!isDefined(nodeValue) && (jsf.formOptions.setSchemaDefaults === true || (jsf.formOptions.setSchemaDefaults === 'auto' && isEmpty(jsf.formValues)))) {
  ...
  switch (controlType) {
    ...
    case 'FormArray':
      ...
      if (isArray(schema.items)) {
        ...
        const tupleItems = nodeOptions.get('tupleItems') || (isArray(schema.items) ? Math.min(schema.items.length, maxItems) : 0);
        for (let i = 0; i < tupleItems; i++) {
          if (i < minItems) {
            controls.push(
              buildFormGroupTemplate(jsf, isArray(nodeValue) ? nodeValue[i] : nodeValue, setValues, schemaPointer + '/items/' + i, dataPointer + '/' + i, templatePointer + '/controls/' + i)
            );
          } else {
            ...
            controls.push(
              isArray(nodeValue)
                ...
                : itemRecursive
                ? null
                : _.cloneDeep(jsf.templateRefLibrary[itemRefPointer])
            );
          }
        }
      ...
      if (additionalItemsPointer) {
        ...
        if (!itemRecursive || hasOwn(validators, 'required')) {
          const arrayLength = Math.min(Math.max(itemRecursive ? 0 : itemOptions.get('tupleItems') + itemOptions.get('listItems') || 0, isArray(nodeValue) ? nodeValue.length : 0), maxItems);
 ...
 * @returnboolean} - true if any fields have been set to required, false if not
 */
 ...
 * @returnany} - formatted data object
 */
export function formatFormData(
  ...
  JsonPointer.forEachDeep(formData, (value, dataPointer) => {
    ...
    if (returnEmptyFields && isArray(value)) {
      ...
      let genericPointer = JsonPointer.has(dataMap, [dataPointer, 'schemaType']) ? dataPointer : removeRecursiveReferences(dataPointer, recursiveRefMap, arrayMap);
      if (JsonPointer.has(dataMap, [genericPointer, 'schemaType'])) {
        const schemaType: SchemaPrimitiveType | SchemaPrimitiveType[] = dataMap.get(genericPointer).get('schemaType');
        if (schemaType === 'null') {
          JsonPointer.set(formattedData, dataPointer, null);
        } else if ((hasValue(value) || returnEmptyFields) && inArray(schemaType, ['string', 'integer', 'number', 'boolean'])) {
          const newValue = fixErrors || (value === null && returnEmptyFields) ? toSchemaType(value, schemaType) : toJavaScriptType(value, schemaType);
        ...
        } else if (schemaType === typeof(Object) && !returnEmptyFields) {
 ...
 * @returngroup} - Located value (or null, if no control found)
 */
