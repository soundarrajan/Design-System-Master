...
export function convertSchemaToDraft6(schema, options: OptionObject = {}) {
  ...
  if (typeof newSchema.$schema === 'string' && /http\:\/\/json\-schema\.org\/draft\-0\d\/schema\#/.test(newSchema.$schema)) {
  ...
  if (typeof newSchema.dependencies === 'object' && Object.keys(newSchema.dependencies).some(key => typeof newSchema.dependencies[key] === 'string')) {
  ...
  if (typeof newSchema.properties === 'object') {
    ...
    if (Object.keys(properties).some(key => properties[key].requires)) {
      ...
      Object.keys(properties)
        ...
        .forEach(key => (dependencies[key] = typeof properties[key].requires === 'string' ? [properties[key].requires] : properties[key].requires));
  ...
  if (newSchema.type && (typeof newSchema.type.every === 'function' ? !newSchema.type.every(type => simpleTypes.includes(type)) : !simpleTypes.includes(newSchema.type))) {
    changed = true;
  }

  // If schema changed, update or remove $schema identifier
  if (typeof newSchema.$schema === 'string' && /http\:\/\/json\-schema\.org\/draft\-0[1-4]\/schema\#/.test(newSchema.$schema)) {
    ...
    if (typeof newSchema.type === 'string') {
      ...
      if (typeof newSchema.type.every === 'function') {
        ...
        if (newSchema.type.every(type => typeof type === 'string')) {
          newSchema.type = newSchema.type.some(type => type === 'any') ? (newSchema.type = simpleTypes) : newSchema.type.filter(type => simpleTypes.includes(type));
          ...
          const objectKeys = ['maxProperties', 'minProperties', 'required', 'additionalProperties', 'properties', 'patternProperties', 'dependencies', 'propertyNames'];
          ...
          for (const type of newSchema.type) {
            ...
            Object.keys(newSchema)
              .filter(key => !newType.hasOwnProperty(key) && ![...(filterKeys[newType.type] || filterKeys.all), 'type', 'default'].includes(key))
  ...
  Object.keys(newSchema)
    ...
    .forEach(key => {
      if (['definitions', 'dependencies', 'properties', 'patternProperties'].includes(key) && typeof newSchema[key].map !== 'function') {
        const newKey = {};
        Object.keys(newSchema[key]).forEach(subKey => (newKey[subKey] = convertSchemaToDraft6(newSchema[key][subKey], { changed, draft })));
