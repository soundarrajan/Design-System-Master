...
export class JsonPointer {
   ...
   * @return object } - Located value (or true or false if getBoolean = true)
   */
  static getCopy(object, pointer, startSlice = 0, endSlice: number = null, getBoolean = false, errors = false) {
   ...
   */
  static forEachDeep(object, fn: (v: any, p?: string, o?: any) => any = v => v, bottomUp = false, pointer = '', rootObject = object) {
   ...
   */
  static forEachDeepCopy(object, fn: (v: any, p?: string, o?: any) => any = v => v, bottomUp = false, pointer = '', rootObject = object) {
    if (typeof fn !== 'function') {
  ...
  static isSubPointer(shortPointer, longPointer, trueIfMatching = false, errors = false) {
    ...
    return shortPointer === longPointer ? trueIfMatching : `${shortPointer}/` === longPointer.slice(0, shortPointer.length + 1);
  }
   ...
   */
  static toIndexedPointer(genericPointer, indexArray, arrayMap: Map<string, number> = null) {
    if (this.isJsonPointer(genericPointer) && isArray(indexArray)) {
      let indexedPointer = this.compile(genericPointer);
      if (isMap(arrayMap)) {
        let arrayIndex = 0;
        return indexedPointer.replace(/\/\-(?=\/|$)/g, (key, stringIndex) => (arrayMap.has((<string>indexedPointer).slice(0, stringIndex)) ? '/' + indexArray[arrayIndex++] : key));
      } else {
    ...
    if (this.isJsonPointer(dataPointer) && typeof schema === 'object') {
      ...
      if (schema.type === 'object' || schema.properties || schema.additionalProperties) {
        if ((schema.properties || {})[firstKey]) {
          return `/properties/${this.escape(firstKey)}` + this.toSchemaPointer(pointerArray, schema.properties[firstKey]);
  ...
  static toDataPointer(schemaPointer, schema, errors = false) {
    if (this.isJsonPointer(schemaPointer) && typeof schema === 'object' && this.has(schema, schemaPointer)) {
      ...
      } else if (['contains', 'definitions', 'dependencies', 'additionalItems', 'additionalProperties', 'patternProperties', 'propertyNames'].includes(firstKey)) {
        if (errors) {
